...
// options：テキストの整形
//====================================
HRESULT DirectWriteCustomFont::DrawString( std::wstring wstr, D3DXVECTOR2 pos, D2D1_DRAW_TEXT_OPTIONS options, bool centering, bool Right_alignment, bool shadow)
{
    HRESULT result = S_OK;
    ...
    //中央揃え
    if( centering ) {
        pounts.x = pos.x - wstr.size() * Setting.fontSize / 2;
        pounts.y = pos.y - Setting.fontSize / 2;
    }
    //右揃え
    else if (Right_alignment)
    {
        pounts.x = (pos.x - wstr.size() * Setting.fontSize / 2) + Setting.fontSize;
        pounts.y = pos.y;
    }
    //通常
    else {
