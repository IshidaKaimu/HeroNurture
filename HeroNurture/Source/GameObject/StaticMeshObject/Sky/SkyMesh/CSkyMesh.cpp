...
//シェーダファイル名(ディレクトリも含む).
const TCHAR SHADER_NAME[] = _T("Data\\Shader\\Sky.hlsl");
...
//メッシュ読み込み.
HRESULT CSkyMesh::LoadXMesh(LPCTSTR lpFileName)
{
	//ファイル名をコピー.
...
//マテリアル作成.
HRESULT CSkyMesh::CreateMaterials()
{
	//ファイルのパス(ディレクトリ)を確保.
...
//インデックスバッファ作成.
HRESULT CSkyMesh::CreateIndexBuffer()
{
...
//頂点バッファ作成.
HRESULT CSkyMesh::CreateVertexBuffer()
{
	D3D11_BUFFER_DESC	bd;	//Dx11バッファ構造体.
	D3D11_SUBRESOURCE_DATA	InitData;//初期化データ.
...
//サンプラ作成.
HRESULT CSkyMesh::CreateSampler()
{
	//テクスチャ用のサンプラ構造体.
...
//コンスタントバッファ作成.
HRESULT CSkyMesh::CreateConstantBuffer()
{
	//コンスタントバッファ(メッシュ用).
